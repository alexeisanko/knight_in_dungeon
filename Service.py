...
def apply_blessing(engine, hero):
    cost = int(20 * 1.5 ** engine.level) - 2 * hero.stats["intelligence"]
    if hero.gold >= cost:
        engine.score += 0.2
        hero.gold -= cost
        ...
        engine.score -= 0.1
        engine.notify(f'For blessing you need still {cost - hero.gold} gold')


def remove_effect(engine, hero):
    cost = int(10 * 1.5 ** engine.level) - 2 * hero.stats["intelligence"]
    if hero.gold >=cost  and "base" in dir(hero):
        hero.gold -= cost
...
def add_gold(engine, hero):
    if random.randint(1, hero.stats['luck']) == 1:
        engine.score -= 0.05
            ...
            for i in range(41):
                for j in range(41):
                    if i == 0 or j == 0 or i == 40 or j == 40:
                        ...
                        self.Map[i][j] = [wall, floor1, floor2, floor3, floor1,
                    ...
                    if obj_name == enemy:
                        for i in range(count):
                            coord = (random.randint(1, 30),
                                     random.randint(1, 22))  # Почему координаты такие (наверно что бы были внутри)
                            intersect = True
                            while intersect:
                                intersect = False
                                if _map[coord[1]][coord[0]] == wall:
